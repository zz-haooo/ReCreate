...
def call_llm(payload):
    ...
    # Venus API support (enabled via VENUS_API_KEY env var)
    if os.environ.get('VENUS_API_KEY'):
        from openai import OpenAI
        base_url = os.environ.get("VENUS_API_BASE_URL")
        if not base_url:
            raise ValueError("VENUS_API_BASE_URL environment variable must be set when using Venus API")
        client = OpenAI(
            api_key=os.environ['VENUS_API_KEY'],
            base_url=base_url
        )
        logger.info("Generating content with Venus API, model: %s", model)
        
        # Convert message format (DA-code uses content: [{type: text, text: ...}] format)
        venus_messages = []
        for msg in payload["messages"]:
            ...
            venus_messages.append({"role": msg["role"], "content": content})
        
        for i in range(3):
            try:
                response = client.chat.completions.create(
                    model=model,
                    messages=venus_messages,
                    max_completion_tokens=payload.get("max_tokens", 1500),
                )
                return True, response.choices[0].message.content
            except Exception as e:
                logger.error("Failed to call Venus API: " + str(e))
                time.sleep(4 * (2 ** (i + 1)))
        return False, "venus_api_error"
